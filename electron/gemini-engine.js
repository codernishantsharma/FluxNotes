...
(function () {
    ...
    try {
        var saved = localStorage.getItem('fluxnotes_sessions');
        if (saved) {
            _sessions = JSON.parse(saved);
            console.log('[fluxnotes API] Restored', Object.keys(_sessions).length, 'sessions from localStorage');
        }
    } catch (e) {
        console.error('[fluxnotes API] Failed to restore sessions:', e);
    }
    ...
    function saveSession(sessionId) {
        ...
        try {
            _pruneSessions();
            localStorage.setItem('fluxnotes_sessions', JSON.stringify(_sessions));
        } catch (e) { }
    }
    ...
    function _detectWorkspaces() {
        try {
            ...
            console.log('[fluxnotes API] Detected Workspaces:', JSON.stringify(_workspaces));
        } catch (e) {
            console.error('[fluxnotes API] Workspace detection error:', e);
        }
    }
    ...
    async function _getTokens(forceRefresh) {
        ...
        try {
            ...
            console.error('[fluxnotes API] Failed to extract tokens from page context:', e);
        }

        // Return immediately if extraction succeeded
        if (at && bl) {
            console.log('[fluxnotes API] Dynamically extracted active tokens from page memory. AT:', at.substring(0, 15) + '...', 'BL:', bl);
            _tokens = { at: at, bl: bl };
            _tokensFetchedAt = Date.now();
            return _tokens;
        }

        console.log('[fluxnotes API] Mismatch/Missing tokens in memory. Fetching fallback tokens from /faq...');
    ...
    function _parseResponse(rawText, commitIds) {
        ...
        if (allItems.length === 0) {
            console.error('[fluxnotes API] Parse error: Raw response length is ' + rawText.length + '. Content: ' + rawText.substring(0, 500));
        ...
        if (!replyText) {
            if (commitIds) {
                console.error('[fluxnotes API] Parse error: ReplyText empty. Raw response: ' + rawText.substring(0, 500));
            }
    ...
    async function uploadFileToGoogle(fileBase64, filename, mimeType) {
        console.log('[fluxnotes Gemini API] Initializing resumable upload for:', filename);
    ...
    async function _processStreamResponse(res) {
        ...
        var rawText = '';
        window.__fluxnotesGeminiStream = { response: '', status: 'streaming', updates: [] };
        ...
        try {
            ...
            window.__fluxnotesGeminiStream.status = 'done';
        } catch (err) {
            window.__fluxnotesGeminiStream.status = 'error';
            window.__fluxnotesGeminiStream.error = err.message;
            throw err;
        }
    ...
    async function send(message, engine, attachments, sessionId, _isRetry) {
        ...
        try {
            ...
            console.log('[fluxnotes API] Query routing - Engine:', engine, 'Workspace:', workspaceId, 'ModelId:', modelId);
            ...
            if (attachments && attachments.imageToken) {
                ...
                console.log('[fluxnotes API] Sending message with attachment - Code:', typeCode, 'Mime:', mime);
            }
            ...
            if (res.status === 400) {
                ...
                if (!res.ok) {
                    if (res.status === 400 && _conversationId) {
                        console.log('[fluxnotes API] Old conversation ID invalid on current account. Resetting session...');
            ...
            try {
                ...
                if (!_isRetry && (_conversationId || _responseId || _choiceId || _contextToken)) {
                    console.log('[fluxnotes API] Continued conversation unparseable/expired. Starting a fresh conversation and retrying once...');
                    newConversation(sessionId);
            ...
            console.error('[fluxnotes API] Chat processing failed. Resetting sticky error state context.', err.message);
            newConversation(sessionId);
            throw err;
        }
    }
    ...
    function newConversation(sessionId) {
        ...
        try {
            ...
            console.warn('[fluxnotes API] Failed to update history state:', historyErr.message);
        }
        console.log('[fluxnotes API] Conversation reset:', sessionId || 'current');
    }

    window.__fluxnotesGeminiUnified = { send: send, newConversation: newConversation };
    window.__fluxnotesGemini = {
    ...
    console.log('[fluxnotes] Upgraded Unified Multimodal Gemini Engine Loaded');
})();